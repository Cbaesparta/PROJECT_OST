...
# handle_login: CORRECTED VERSION - Displays a form to get user credentials and logs them in.
handle_login() {
    # This dialog uses --passwordform, which is more reliable for simple user/pass entry.
    dialog --backtitle "Railway Ticketing System" \
           --title "User Login" \
           --clear \
           --passwordform "Enter your credentials:" 12 50 0 \
           "Username:" 1 1 "" 1 10 25 0 \
           "Password:" 2 1 "" 2 10 25 0 \
           2>"$output_file"

    # Check if the user pressed Cancel or Escape
    [ $? -ne 0 ] && return

    # Read the captured values from the temporary file.
